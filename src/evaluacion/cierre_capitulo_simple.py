...
def verificar_cierre_automatico(capitulo, columna, repo_dir, registro_path, fecha_limite):
    ...
    # Asegurarse de que ambas fechas tienen timezone
    madrid_tz = pytz.timezone('Europe/Madrid')
    
    # Obtener hora actual en Madrid
    ahora = datetime.now(madrid_tz)
    
    # Asegurar que fecha_limite también tiene timezone de Madrid
    if fecha_limite.tzinfo is None:
        # Si no tiene timezone, añadirla
        fecha_limite = madrid_tz.localize(fecha_limite)
    else:
        # Si ya tiene timezone, convertir a Madrid por si acaso
        fecha_limite = fecha_limite.astimezone(madrid_tz)
    
    print(f"\n{'='*80}")
    print(f"🔍 VERIFICACIÓN DE CIERRE AUTOMÁTICO")
    print(f"{'='*80}")
    print(f"Ahora (Madrid):        {ahora.strftime('%Y-%m-%d %H:%M:%S %Z')}")
    print(f"Fecha límite (Madrid): {fecha_limite.strftime('%Y-%m-%d %H:%M:%S %Z')}")
    print(f"¿Pasó la fecha límite? {ahora > fecha_limite}")
    print(f"Diferencia: {ahora - fecha_limite}")
    print(f"{'='*80}\n")
    
    # Solo cierra si pasó la fecha límite
    if ahora <= fecha_limite:
        minutos_restantes = int((fecha_limite - ahora).total_seconds() / 60)
        print(f"⏰ Plazo NO vencido. Faltan {minutos_restantes} minutos")
        return False

    # Verifica si ya se cerró (comprobando si hay emojis especiales)
    full_path = os.path.join(repo_dir, registro_path)
    if not os.path.exists(full_path):
        print(f"⚠️ Archivo {registro_path} no encontrado")
        return False
    ...
    emojis_hall = ["🏆", "📝", "🔍", "🤖"]
    valores = df[columna].astype(str).values

    if any(v in emojis_hall for v in valores):
        # Ya se cerró antes
        print(f"⚠️ Ya se había cerrado antes (emojis encontrados)")
        return False

    # Cierra automáticamente
    print("✅ PLAZO VENCIDO - Cerrando capítulo automáticamente...")
